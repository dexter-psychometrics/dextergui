	...
	$('body').click( 
		function( event ) 
		{
			if(event.altKey)
			{
				...
				// don't zoom in on a cell, take the row
				if(target.closest('tr').length > 0)
				{
					target = $(target).closest('tr');				
				} else
				{
					// don't zoom in on any very small thing, it's not helpfull
					while(target.height() < 50 || target.width() < 50) target = target.parent();
				}
				
				zoom.to({ element: target.get(0) });
			}
		}
	);
